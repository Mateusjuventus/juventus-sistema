 ...
 * tem lá dentro, em vez de só um crest redundante + "Entrar".
 */
export default async function HomePage() {
  ...
  return (
    <AppShell nav="none">
      <div className="flex min-h-[70vh] flex-col items-center justify-center px-2 py-8">
        <div className="relative">
          <div aria-hidden className="absolute inset-0 -z-10 rounded-full bg-dourado/15 blur-2xl" />
          <div className="rounded-full border border-dourado/40 bg-white p-3 shadow-sm">
            <JuventusCrest className="h-16 w-auto sm:h-20" />
          </div>
        </div>
        <h1 className="mt-6 text-center text-3xl font-bold tracking-tight text-grena-escuro sm:text-4xl">
          Juventus - SAF
        </h1>
          ...
          <div className={`mt-10 grid w-full gap-5 ${temAmbos ? "max-w-3xl sm:grid-cols-2" : "max-w-sm"}`}>
            {temProfissional ? (
              ...
              >
                ...
                <span className="mt-5 inline-flex items-center gap-1 text-sm font-semibold text-grena">
                  Entrar
