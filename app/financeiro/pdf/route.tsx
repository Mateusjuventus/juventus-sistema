...
import {
  ...
  type RelatorioPdfDespesaAvulsa,
...
export async function GET() {
  ...
  const [{ data: jogosData }, { data: gastosData }, { data: despesasAvulsasData }, { assinatura1, assinatura2 }] =
    await Promise.all([
  ...
  if (gastos.length === 0 && despesasAvulsas.length === 0) {
    return new NextResponse("Ainda não há gastos lançados em nenhum jogo.", { status: 400 });
  }

  const totalPrevisto =
    gastos.reduce((soma, g) => soma + g.valor_previsto, 0) +
    despesasAvulsas.reduce((soma, d) => soma + d.valor_previsto, 0);
  const totalEfetuado =
    gastos.reduce((soma, g) => soma + (g.valor_efetuado ?? 0), 0) +
    despesasAvulsas.reduce((soma, d) => soma + (d.valor_efetuado ?? 0), 0);
  ...
  const despesasAvulsasPdf: RelatorioPdfDespesaAvulsa[] = despesasAvulsas.map((d) => ({
  ...
  const buffer = await renderToBuffer(
    <RelatorioFinanceiroDocument
      ...
      jogos={jogosPdf}
      despesasAvulsas={despesasAvulsasPdf}
