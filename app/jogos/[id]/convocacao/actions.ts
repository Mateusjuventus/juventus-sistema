...
export async function saveConvocacao(
  ...
  await Promise.all([
    supabase.from("convocacao_atletas").delete().eq("convocacao_id", convocacaoId),
    supabase.from("convocacao_comissao").delete().eq("convocacao_id", convocacaoId),
    supabase.from("convocacao_staff").delete().eq("convocacao_id", convocacaoId),
  ]);
  ...
  if (atletaStatus.length > 0) {
    inserts.push(
      Promise.resolve(
        supabase.from("convocacao_atletas").insert(
          atletaStatus.map((a) => ({
  ...
  if (comissaoIds.length > 0) {
    inserts.push(
      Promise.resolve(
        supabase.from("convocacao_comissao").insert(
  ...
  if (staffIds.length > 0) {
    inserts.push(
      Promise.resolve(
        supabase.from("convocacao_staff").insert(
  ...
  revalidatePath(`/jogos/${jogoId}/convocacao`);
  return { success: true };
}
