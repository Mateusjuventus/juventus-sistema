...
export default async function VagasStaffPage({ params }: { params: { id: string } }) {
  ...
  // Agrupa quem já pegou vaga por função, pra tela mostrar uma seção por função em vez de uma lista
  // só — com várias funções abertas, encontrar todo mundo da Segurança misturado com o resto virava
  // trabalho de procurar linha por linha.
  const confirmadosPorFuncao = new Map<string, JogoVagasStaffInscricaoRow[]>();
  const esperaPorFuncao = new Map<string, JogoVagasStaffInscricaoRow[]>();
