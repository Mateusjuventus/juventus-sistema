...
/** Lista de Jogos do Futebol de Base filtrada por categoria (Sub20 a Sub11) — espelha
 * `app/jogos/page.tsx`, mas sempre restrita à categoria da URL (ver a spec). */
export default async function JogosBaseCategoriaPage({
  params,
  searchParams,
}: {
  ...
  const [{ data, error }, { data: todosJogosData }, { data: convocacoesData }] = await Promise.all([
    query,
    supabase.from("jogos_base").select("id, data_jogo").eq("categoria", categoria),
  ...
  return (
    <AppShell departamento="futebol_base">
      ...
      <PageHeader title={`Jogos / Competições — ${categoriaBaseLabel(categoria)}`} pendencia={pendenciaJogos} />
      <div className="mt-3 flex flex-wrap justify-end gap-2">
        <Link
          href={`/base/jogos/${categoria}?q=${encodeURIComponent(q)}&mandante=${encodeURIComponent(mandanteFiltro)}&ordem=${
        ...
        <a
          href={`/base/jogos/${categoria}/export?q=${encodeURIComponent(q)}&mandante=${encodeURIComponent(mandanteFiltro)}`}
          className="btn-secondary"
        >
          Exportar para Excel
        </a>
        <Link href={`/base/jogos/${categoria}/novo`} className="btn-primary">
          + Novo jogo
        </Link>
      </div>

      <div className="card mt-4 p-4">
        <SearchBar action={`/base/jogos/${categoria}`} defaultValue={q} placeholder="Buscar por adversário...">
          ...
          return (
            <div key={j.id} className="card flex flex-col gap-4 p-5">
              ...
              <div className="flex flex-wrap justify-center gap-2 border-t border-neutral-100 pt-3">
                <Link href={`/base/jogos/${categoria}/${j.id}/convocacao`} className="btn-secondary">
                  Entrar
                </Link>
                <Link href={`/base/jogos/${categoria}/${j.id}/checklist`} className="btn-secondary">
                  Checklist
                </Link>
                <Link href={`/base/jogos/${categoria}/${j.id}`} className="btn-secondary">
                  Editar
                </Link>
