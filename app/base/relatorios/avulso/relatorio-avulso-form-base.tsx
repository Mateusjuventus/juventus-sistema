...
/**
 * Espelha `app/relatorios/avulso/relatorio-avulso-form.tsx` para o Futebol de Base — mesmo
 * formulário, mais um checkbox de Categoria (Sub-20 etc.) já que só o Futebol de Base tem essa
 * informação nos cadastros. Monta um PDF sob medida (título, descrição, informações de jogo
 * opcionais — digitadas ou preenchidas a partir de um jogo já cadastrado —, quem entra e quais
 * colunas aparecem) sem depender de nenhum jogo já convocado. Envio é um POST simples de formulário
 * (sem Server Action) — abre o PDF numa aba nova, igual a qualquer link "Gerar PDF" do sistema, só
 * que apontando pra uma rota que lê os dados do corpo do POST em vez de um `id` de jogo na URL.
 */
export function RelatorioAvulsoFormBase({
  actionUrl,
  jogosCadastrados,
  atletas,
  comissao,
