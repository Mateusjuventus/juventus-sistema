...
export async function GET(request: NextRequest) {
  ...
  const linhas = solicitacoes.map((s) => ({
    ...
    // Em Pagamento/Reembolso, Valor é a soma automática dos itens — o detalhe item a item
    // (compra, pagamentos, passageiros) sai na aba "Itens", logo abaixo.
    Valor: s.valor ?? "",
  ...
  // Aba "Itens" — detalhe item a item (Compra, Pagamento/Reembolso, Passagem Aérea) de todas as
  // solicitações filtradas, pra dar controle completo do que está sendo pedido sem precisar abrir
  ...
  return buildXlsxResponse("solicitacoes.xlsx", [
