 ...
 *
 * Treinador só enxerga a(s) categoria(s) em que atua (`perfis.categorias_treinador`); qualquer
 * pessoa da Base (master, ou regular com o departamento Futebol de Base liberado) enxerga as 7.
 * Ver docs/superpowers/specs/2026-08-30-area-treinador-programacao-design.md, "Permissões (RLS)"
 * — esta função é a trava de verdade, chamada de novo em toda query/action antes de ler/gravar;
 * RLS nas tabelas `programacao_*` é só a mesma última linha de defesa genérica que protege todas
 * as outras tabelas `*_base` do sistema (policy única pra qualquer usuário autenticado), não uma
 * restrição por categoria.
 *
 * Desde 30/08 a Programação deixou de ser um módulo com checkbox próprio em `/usuarios` — virou a
 * Home do Futebol de Base (`app/base/page.tsx`), então qualquer pessoa com o departamento liberado
 * já enxerga e edita as 7 categorias, sem permissão extra (antes exigia o módulo "Programação"
 * liberado; ver histórico de `lib/auth/modulos-base.ts`).
 */
